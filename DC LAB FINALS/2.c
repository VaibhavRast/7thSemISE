...
int Matrix[NoofRows][NoofCols],Vector[Vectorsize], Result[Vectorsize], Result1[Vectorsize] ;
...
// for (i = 0; i<NoofRows; i = i + 1) 
// 	for (j = 0; j <NoofCols; j = j + 1) 
// 		Result1[i] = Result1[i] + Matrix[i][j] * Vector[j]; 
// for (i = 0; i<NoofRows; i++) 
// 	printf("%d \n", Result1[i]); 
// Comare serial and parallel result if asked.
} 
